            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header" v-if="!isSelectingMessages">
                        {{ user.name }}
                        <sup v-if="isOnline" class="online-dot"></sup>
                        <sup v-else class="offline-dot"></sup>
                        <small v-if="isUserTyping" class="text-muted mt-2" style="color: #000ff0 !important;">
                                typing...
                        </small>

                        <a :href="`/groups/${currentUser.id}`" class="btn btn-primary btn-sm float-end">Groups</a>
                    ...
                    <div class="card-body chat-body" ref="messageContainer">
                        <div class="d-flex flex-column" v-for="(messages, date) in groupedMessages" :key="date">
                            ...
                            <div v-for="message in messages" :key="message.id"
                                :class="message.sender_id === currentUser.id ? 'align-self-end senderSide' : 'align-self-start receiverSide'" class="d-flex mb-3 message-container"  @mouseenter="markAsRead(message.id)">
                                ...
                                <div class="position-relative">
                                    <div v-if="message.is_deleted==1" class="text-muted small" style="font-style: italic;">
                                        This message is deleted by {{ user.name }}
                                    ...
                                    <div v-else>
                                        ...
                                        <div v-if="message.replied_to" class="replied-message-container mb-2">
                                            <div v-if="message.replied_to.is_deleted==0" class="replied-message-content p-2">
                                                <strong class="replied-message-sender"><i class="fa-solid fa-reply"></i> {{ getMessageSenderName(message.replied_to.sender_id) }}:</strong>
                                                <span v-if="message.replied_to.text" class="replied-message-text">{{ message.replied_to.text }}</span>
                                                <div v-else-if="message.replied_to.attachment" class="file-download">
                                                    <div v-if="isImage(message.replied_to.attachment)">
                                                        <a :href="`/${message.replied_to.attachment}`" :download="message.replied_to.attachment">
                                                    ...
                                                    <div v-else-if="isAudio(message.replied_to.attachment)" style="position: relative; display: inline-block;">
                                                        <audio
                                                            :ref="el => audioRefs.set(message.replied_to.id, el)"
                                                                        ...
                                                                        <div class="col-md-12">
                                                                            <div class="row">
                                                                                ...
                                                                                <div class="col-md-10" style="text-align: left;">
                                                                                    <span class="card-title" style="font-size: 12px; font-weight: 700;">{{ message.replied_to.attachment}}</span><br>
                                        ...
                                        <div v-else>
                                            <div v-if="message.attachment" class="file-download">
                                                ...
                                                <div class="message-reactions">
                                                    <span
                                                        v-for="reaction in message.message_reaction"
                                                        :key="reaction.id"
                                            ...
                                            <div v-if="message.text" :class="message.sender_id === currentUser.id ? 'text-white message-bubble-out' : 'bg-light message-bubble-in'" class="px-3 py-1 border rounded">
                                            ...
                                            <div v-else class="dropdown position-absolute top-0 end-0 p-1">
                                                <button v-if="message.is_deleted==0" class="btn btn-link p-0" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                                                    <i class="fa fa-ellipsis-v text-primary"></i>
                                                </button>
                                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
                                                    <li><a class="dropdown-item" href="#" @click.prevent="replyToMessage(message.id)">Replay</a></li>
                        ...
                        <img v-if="isUserTyping"  src="/typing.gif" alt="Typing..." class="typing-indicator">
                    ...
                    <div v-if="repliedMessage"
                        ...
                        style="max-width: 30%; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
                        <div class="d-flex align-items-start">
                            <div>
                                ...
                                <div v-else-if="repliedMessage.attachment" class="text-secondary" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px;">
                                    "File"
                                </div>
                            </div>
                            <button
                    ...
                    <div class="card-footer" v-if="!selectedMessages.length > 0">
                        <form @submit.prevent="sendMessage">
                            <div class="input-group">
                                ...
                                <input v-if="!isRecording" v-model="newMessage" @keydown="sendTypingEvent" type="text" class="form-control" placeholder="Type a message..." aria-label="Message">
...
const isOnline = computed(() => {
    return onlineUsers.value.some(u => u.id === props.user.id);
});
...
const copySelectedMessages = () => {
    const formattedMessages = selectedMessages.value.map(msgId => {
        ...
        const senderName = message.sender_id === props.currentUser.id ? props.currentUser.name : props.user.name;
...
const deleteSelectedMessages = async () => {
    try {
        ...
        const response = await axios.delete('/delete/multiple/messages', {
            data: {
...
const addReaction = async (emoji, messageId) => {
    ...
    try {
        const response = await axios.post(`http://127.0.0.1:8000/messages/reactions/${props.currentUser.id}`, formData, {
            headers: {
...
const removeReaction = async (reaction) => {
    ...
    try {
        const response = await axios.delete(`http://127.0.0.1:8000/messages/reactions/${reaction.id}`, {
            headers: {
        ...
        if (response.data.success) {
            messages.value = messages.value.map(message => {
                if (message.id === reaction.message_id) {
                    return {
                        ...message,
                        message_reaction: message.message_reaction.filter(r => r.id !== reaction.id)
                    };
                }
                return message;
            });
    ...
    if (targetId && messageToForward.value) {
        try {
            ...
            const url = targetType === 'user'
                ? 'http://127.0.0.1:8000/messages/forward'
                : 'http://127.0.0.1:8000/messages/forward-group';
...
const sendMessage = async () => {
    if (newMessage.value.trim() !== '' || file.value || audioBlob.value) {
        ...
        try {
            ...
            const response = await axios.post(`http://127.0.0.1:8000/messages/${props.user.id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            const messageExists = messages.value.some(message => message.id === response.data.id);
            if (!messageExists) {
...
const getMessageSenderName = (senderId) => {
    if (senderId === props.currentUser.id) {
        ...
        const user = props.user; // Assuming props.user is the other user in the chat
        return user.name;
    }
};
...
const markAsRead = async (messageId) => {
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        await axios.post(`http://127.0.0.1:8000/messages/${messageId}/read`, {
            _token: csrfToken
        });
    } catch (error) {
...
const deleteMessage = async (messageId) => {
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        await axios.put(`http://127.0.0.1:8000/messages/delete/${messageId}`, {
...
const hideMessage = async (messageId) => {
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        await axios.post(`http://127.0.0.1:8000/messages/hide/${messageId}`, {
            data: { _token: csrfToken }
        });
        messages.value = messages.value.filter(message => message.id !== messageId);
    } catch (error) {
...
const updateMessage = async (messageId) => {
    if (editedMessageText.value.trim() !== '') {
        try {
            const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            const response = await axios.put(`http://127.0.0.1:8000/messages/edit/${messageId}`, {
...
const clearChat = async () => {
    ...
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        await axios.post(`http://127.0.0.1:8000/messages/clear/${props.user.id}`, {
            _token: csrfToken
        });
...
onMounted(() => {
    ...
    Echo.private(`chat.${props.currentUser.id}`)
        .listen("MessageSent", (response) => {
            const newMessage = { ...response.message, is_deleted: 0 }; // Set is_deleted to 0
        ...
        .listen('MessageDelivered', (event) => {
            messages.value = messages.value.map(message =>
                message.id === event.message.id ? { ...message, status: 'delivered' } : message
            );
        })
        .listen('MessageRead', (event) => {
            if (event.message.sender_id === props.currentUser.id) {
                messages.value = messages.value.map(message =>
                    message.id === event.message.id ? { ...message, status: 'read' } : message
                );
            }
        })
    ...
    .listen('MessageHide', (event) => {
        fetchMessages();
    });

    Echo.private(`chat.${props.currentUser.id}`)
    .listen("MessageUpdate", (response) => {
        messages.value = messages.value.map(message =>
